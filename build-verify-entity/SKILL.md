...
description: GcTDEntity 编译验证 — 先 Debug 后 Release，异步执行，修复编译错误
---
...
```powershell
pwsh -NoProfile -Command "& { .\BuildDebug.ps1; exit $LASTEXITCODE }"
```
...
```powershell
pwsh -NoProfile -Command "& { .\BuildRelease.ps1; exit $LASTEXITCODE }"
```
