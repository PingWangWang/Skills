...
> 一套 Claude Code 代码质量工作流技能 — 从分析、编码、审查到提交，覆盖全流程。面向中文 Claude Code 用户，即装即用。
...
| 技能 | 用途 | 触发场景 |
|------|------|---------|
| `code-design` | 方案设计 — 理解需求、探索项目、产出多方案选型 | 新功能设计、架构选型、技术方案评审 |
...
| `build-verify-entity` | GcTDEntity 编译 — Debug → Release 顺序编译，NoWait 异步执行，修复编译错误 | GcTDEntity 编译、Entity 构建出错 |
| `build-verify-dt` | GcThermalDesktop 编译验证 — UpdateSDK → BuildDebug → BuildRelease，修复编译错误 | GcThermalDesktop 编译、Desktop 构建出错 |
| `build-verify-td` | Thermal Desktop 全流程编译 — GcTDEntity → GcThermalDesktop 串联编译，修复编译错误 | TD 全量编译、构建出错 |
...
**独立技能**：`readme-zh`、`build-verify-entity`、`build-verify-dt`、`build-verify-td` 可随时调用，不依赖其他技能。

- **`code-design`** 是设计入口 — 新功能先理解需求、调研项目、出多方案，选型后再进入后续流程
...
```
帮我设计一下这个新功能              → 触发 code-design
帮我分析一下这个崩溃的原因          → 触发 code-analysis
直接帮我修这个 Bug，不用确认        → 触发 code-writting
编译一下 Entity                        → 触发 build-verify-entity
编译一下 Desktop                       → 触发 build-verify-dt
编译一下 TD 项目                       → 触发 build-verify-td
...
│   └── SKILL.md
├── build-verify-entity/ # GcTDEntity 编译构建
│   └── SKILL.md
├── build-verify-dt/     # GcThermalDesktop 编译构建
│   └── SKILL.md
├── build-verify-td/     # TD 全流程编译（Entity + Desktop）
│   └── SKILL.md
...
- **Thermal Desktop 开发者**：用 `build-verify-entity`、`build-verify-dt`、`build-verify-td` 一键编译并修复错误
